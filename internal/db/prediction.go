...
func GetPlayersPredictionOnMatch(ctx context.Context, match *models.Match) ([]*models.Prediction, error) {
	...
	err := db.Select(&predictions, "SELECT elo,discordID,twitchID,osuser,lastrankupdate,team,credits,amount FROM players JOIN predictions ON predictions.playerID = players.discordID WHERE matchID=? ", match.ID)
	if err != nil {
