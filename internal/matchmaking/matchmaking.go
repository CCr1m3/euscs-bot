...
func Init() {
	...
	if os.Getenv("mode") == "dev" {
		...
		for i := 0; i < 30; i++ {
			...
			err = player.SetOSUser(ctx, player.OSUser)
			if err != nil {
				log.Error(err)
			}
...
func zeroFlexGoaliesSample(forwards int, flex int, goalies int) [6]int {
	indices := [6]int{}
	indices[0] = random.FastRandN(goalies)
	indices[1] = random.FastRandN(goalies - 1)
	if indices[1] >= indices[0] {
		indices[1]++
	}
	indices[2] = random.FastRandN(forwards+flex) + goalies
	indices[3] = random.FastRandN(forwards+flex-1) + goalies
	if indices[3] >= indices[2] {
		indices[3]++
	}
	indices[4] = random.FastRandN(forwards+flex) + goalies
	for indices[4] == indices[3] || indices[4] == indices[2] {
		indices[4] = random.FastRandN(forwards+flex) + goalies
	}
	indices[5] = random.FastRandN(forwards+flex) + goalies
...
func oneFlexGoalieSample(forwards int, flex int, goalies int) [6]int {
	indices := [6]int{}
	indices[0] = random.FastRandN(goalies)
	indices[1] = random.FastRandN(flex) + goalies
	indices[2] = random.FastRandN(flex+forwards-1) + goalies
	if indices[2] >= indices[1] {
		indices[2]++
	}
	indices[3] = random.FastRandN(forwards+flex) + goalies
...
func twoFlexGoaliesSample(forwards int, flex int, goalies int) [6]int {
	indices := [6]int{}
	indices[0] = random.FastRandN(flex) + goalies
	indices[1] = random.FastRandN(flex-1) + goalies
	if indices[1] >= indices[0] {
		indices[1]++
	}
	indices[2] = random.FastRandN(forwards+flex) + goalies
...
func evaluatePlayers(indices *[6]int, players []*db.QueuedPlayer) float64 {
	const eloRange = 500
...
func evaluateTeams(team1 []*db.Player, team2 []*db.Player) float64 {
...
func balanceTeams(indices *[6]int, players []*db.QueuedPlayer) ([]*db.Player, []*db.Player) {
	...
	for _, split := range fwdsSplit {
		team1 := []*db.Player{&players[indices[0]].Player, &players[indices[split[0]+1]].Player, &players[indices[split[1]+1]].Player}
		team2 := []*db.Player{&players[indices[1]].Player, &players[indices[split[2]+1]].Player, &players[indices[split[3]+1]].Player}
	...
	team1 := []*db.Player{&players[indices[0]].Player, &players[indices[bestSplit[0]+1]].Player, &players[indices[bestSplit[1]+1]].Player}
	team2 := []*db.Player{&players[indices[1]].Player, &players[indices[bestSplit[2]+1]].Player, &players[indices[bestSplit[3]+1]].Player}
...
func algorithm(ctx context.Context) ([]*db.Player, []*db.Player) {
	playersInQueue, err := db.GetPlayersInQueue(ctx)
	if err != nil {
		log.Error(err)
	}
